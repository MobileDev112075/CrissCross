...
//
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 50, self.view.width, self.view.height-50)];
...
//
        ...
        _btnCam = [[UIButton alloc] initWithFrame:CGRectMake(0, 20, 44, 44)];
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    if(textField == _inputEmail){
        ...
        [_inputPass resignFirstResponder];
        [_inputPass2 resignFirstResponder];
        [self doSave];
...
-(void)sendToServer{
    ...
    if(![_inputEmail.text isEmail]){
        [self showAlertWithTitle:@"Email required" andMessage:@"Please enter a valid email address"];
        [_inputEmail becomeFirstResponder];
        return;
    }
    
    if([_inputPass.text length] < 6){
        [self showAlertWithTitle:@"Password Length" andMessage:@"Please enter a password of 6 or more characters"];
        [_inputPass becomeFirstResponder];
        return;
    }
    
    if(![_inputPass.text isEqualToString:_inputPass2.text]){
        [self showAlertWithTitle:@"Password do not match" andMessage:@"Please re-enter your password"];
        [_inputPass becomeFirstResponder];
        return;
    }
    
    if(!_avatarChanged){
        [self showAlertWithTitle:@"You forgot to add a photo!" andMessage:@"Add a photo to continue"];
