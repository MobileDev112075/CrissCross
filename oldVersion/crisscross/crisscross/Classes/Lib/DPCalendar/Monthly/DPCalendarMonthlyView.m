            ...
            for (DPCalendarEvent *event in events) {
                ...
                if([rowIndexs count] > 1){
                    ...
                    if([rowIndexs count] == 2){
                        DPCalendarEvent *eventFound1 = [otherEventsInTheSameDay firstObject];
                        DPCalendarEvent *eventFound2 = [otherEventsInTheSameDay lastObject];
                ...
                if(conflictFound){
                    for (DPCalendarEvent *eventFound in otherEventsInTheSameDay) {
                        if (eventFound.rowIndex && eventFound.rowIndex < (rowIndexs.count + 1)) {
                            [rowIndexs setObject:[NSNumber numberWithInt:1] atIndexedSubscript:(eventFound.rowIndex - 1)];
                            [[NSNotificationCenter defaultCenter] postNotificationName:NOTIFICATION_PLAN_CONFLICT_ITEM object:eventFound];
                        }
                    }
                    [[NSNotificationCenter defaultCenter] postNotificationName:NOTIFICATION_PLAN_CONFLICT_ITEM object:event];
                }
