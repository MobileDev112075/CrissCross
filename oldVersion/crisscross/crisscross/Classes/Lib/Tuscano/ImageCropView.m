...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        contentView.backgroundColor = [UIColor colorWithHexString:@"000000"];
        
        
        float btnWidth = roundf([AppController sharedInstance].screenBoundsSize.width/2);
        UIButton *btnCancel = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, btnWidth, 60)];
        btnCancel.backgroundColor = [[UIColor colorWithHexString:COLOR_CC_TEAL] colorWithAlphaComponent:1];
        [btnCancel setTitle:@"Cancel" forState:UIControlStateNormal];
        [btnCancel setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [btnCancel addTarget:self action:@selector(cancel:) forControlEvents:UIControlEventTouchUpInside];
        [contentView addSubview:btnCancel];
        
        UIButton *btnCrop = [[UIButton alloc] initWithFrame:CGRectMake(btnWidth + 1, 0, btnWidth, 60)];
        btnCrop.backgroundColor = [[UIColor colorWithHexString:COLOR_CC_TEAL] colorWithAlphaComponent:1];
        [btnCrop setTitle:@"Crop" forState:UIControlStateNormal];
        [btnCrop setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [btnCrop addTarget:self action:@selector(done:) forControlEvents:UIControlEventTouchUpInside];
        
        UILabel *msg = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, [AppController sharedInstance].screenBoundsSize.width, 20)];
        msg.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:1];
        msg.textColor = [UIColor whiteColor];
        msg.textAlignment = NSTextAlignmentCenter;
        msg.font = [UIFont fontWithName:FONT_HELVETICA_NEUE_MED size:12];
        msg.text = @"Move and Crop photo";
        
        btnCancel.y = btnCrop.y = [AppController sharedInstance].screenBoundsSize.height - btnCancel.height;
        msg.y = btnCancel.y - msg.height;
        [contentView addSubview:btnCrop];
        [contentView addSubview:msg];

        CGRect view = CGRectMake(0, 20, [AppController sharedInstance].screenBoundsSize.width, [AppController sharedInstance].screenBoundsSize.height - msg.height - btnCancel.height - 20);
...
//        self.color = [UIColor colorWithRed:18.0/255.0 green:173.0/255.0 blue:251.0/255.0 alpha:1];
        self.color = [UIColor colorWithHexString:COLOR_CC_TEAL];
        self.opaque = NO;
    }
    return self;
}
...
- (UIView*)  checkHit:(CGPoint)point {
    ...
    for (int i =0; i < PointsArray.count; i++) {
        UIView *thePoint = PointsArray[i];

        
        if (sqrt(pow((point.x-view.center.x),2) + pow((point.y-view.center.y),2))>sqrt(pow((point.x- [thePoint center].x),2) + pow((point.y- [thePoint center].y),2)))
            view = PointsArray[i];
         }
